            ...
            <div className="hidden lg:block fixed top-5 left-0 right-0 z-50 px-6">
                <div className={cn(
                    ...
                    isOverLightBackground
                        ...
                        : "bg-brand-black/40 border border-white/10 shadow-black/10"
                )}>
                    {/* Logo with Purple Background */}
                    <Link
                        href="/"
                        className="bg-brand-purple hover:bg-brand-deep text-white px-6 py-3 rounded-full shadow-lg shadow-brand-purple/20 transition-all flex-shrink-0 flex items-center justify-center"
                    >
                    ...
                    <div className="flex-1 flex justify-center">
                        <PillNav
                            navItems={NAV_ITEMS}
                            baseColor={isOverLightBackground ? "rgba(255, 255, 255, 0.9)" : "rgba(255, 255, 255, 0.05)"}
                            pillColor="#6a0eac"
                    ...
                    <Link
                        href="/contact"
                        className="btn-primary px-8 py-3 text-[12px] whitespace-nowrap flex-shrink-0"
                    >
                        Inquire Now
                    </Link>
                </div>
                ...
                className={cn(
                    ...
                    isScrolled
                        ? isOverLightBackground
                            ...
                            : "bg-brand-black/80 backdrop-blur-xl border-b border-white/10 py-4 shadow-lg shadow-black/10"
                        : "bg-transparent"
                )}
            >
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <Link
                        href="/"
                        className="bg-brand-purple hover:bg-brand-deep text-white px-4 py-2.5 rounded-full shadow-lg shadow-brand-purple/20 transition-all flex items-center justify-center"
                    >
                        <div className="h-8 w-28 relative">
                            <img
                                src="https://vsafsub.macadz.com/wp-content/uploads/2026/01/logo-bg.png"
                    ...
                    <button
                        className={cn(
                            "p-2 transition-colors hover:text-brand-purple",
                            isScrolled
                                ? isOverLightBackground ? "text-brand-black" : "text-white"
                                : "text-white"
                        )}
                ...
                {/* Mobile Menu */}
                {mobileMenuOpen && (
                    <div className="absolute top-full left-4 right-4 glass-card border border-white/10 rounded-3xl p-8 mt-4 shadow-2xl flex flex-col space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        {NAV_ITEMS.map((item) => (
                            <Link
                                key={item.label}
                                href={item.href}
                        ...
                        <Link
                            href="/contact"
                            className="btn-primary px-6 py-4 text-center rounded-2xl"
                            onClick={() => setMobileMenuOpen(false)}
                        >
                            Get Started
                        </Link>
